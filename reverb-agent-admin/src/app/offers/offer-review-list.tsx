...
export function OfferReviewList({ reviews }: { reviews: OfferReview[] }): React.JSX.Element {
  ...
  const [counterAmounts, setCounterAmounts] = useState<Record<string, string>>(() => Object.fromEntries(reviews.map((review) => [review.auditId, review.recommendedCounterAmount ?? ""] )));
  ...
  return <>{message ? <p {...stylex.props(styles.error)}>{message}</p> : null}<div {...stylex.props(styles.tableWrap)}><div {...stylex.props(styles.desktopOnly)}><table {...stylex.props(styles.table)}><thead><tr>{["Listing", "Offer", "Recommended", "Action"].map((heading) => <th key={heading} {...stylex.props(styles.cell, styles.heading)}>{heading}</th>)}</tr></thead><tbody>{reviews.map((review) => <tr key={review.auditId}><td {...stylex.props(styles.cell)}>{review.listingId}</td><td {...stylex.props(styles.cell, styles.amount)}>${Number(review.amount).toFixed(2)} {review.currency}</td><td {...stylex.props(styles.cell, styles.recommendation)}>{review.recommendedDecision}{review.recommendedCounterAmount ? ` at $${review.recommendedCounterAmount}` : ""}</td><td {...stylex.props(styles.cell)}><div {...stylex.props(styles.controls)}><button type="button" disabled={busy === review.auditId} onClick={() => act(review, "accept")} {...stylex.props(styles.button)}>Accept</button><button type="button" disabled={busy === review.auditId} onClick={() => act(review, "decline")} {...stylex.props(styles.button, styles.danger)}>Decline</button><div {...stylex.props(styles.counter)}><input aria-label={`Counter amount for ${review.listingId}`} inputMode="decimal" placeholder="Amount" value={counterAmounts[review.auditId] ?? ""} onChange={(event) => setCounterAmounts((current) => ({ ...current, [review.auditId]: event.target.value }))} {...stylex.props(styles.input)} /><button type="button" disabled={busy === review.auditId} onClick={() => act(review, "counter")} {...stylex.props(styles.button)}>Counter</button></div></div></td></tr>)}</tbody></table></div><div {...stylex.props(styles.mobileCards)}>{reviews.map((review) => <article key={review.auditId} {...stylex.props(styles.mobileCard)}><div {...stylex.props(styles.mobileRow)}><span {...stylex.props(styles.mobileLabel)}>Listing</span><span {...stylex.props(styles.mobileValue)}>{review.listingId}</span></div><div {...stylex.props(styles.mobileRow)}><span {...stylex.props(styles.mobileLabel)}>Offer</span><span {...stylex.props(styles.mobileValue)}>${Number(review.amount).toFixed(2)} {review.currency}</span></div><div {...stylex.props(styles.mobileRow)}><span {...stylex.props(styles.mobileLabel)}>Recommended</span><span {...stylex.props(styles.mobileValue, styles.recommendation)}>{review.recommendedDecision}{review.recommendedCounterAmount ? ` at $${review.recommendedCounterAmount}` : ""}</span></div><div {...stylex.props(styles.controls)}><button type="button" disabled={busy === review.auditId} onClick={() => act(review, "accept")} {...stylex.props(styles.button)}>Accept</button><button type="button" disabled={busy === review.auditId} onClick={() => act(review, "decline")} {...stylex.props(styles.button, styles.danger)}>Decline</button><div {...stylex.props(styles.counter)}><input aria-label={`Counter amount for ${review.listingId}`} inputMode="decimal" placeholder="Amount" value={counterAmounts[review.auditId] ?? ""} onChange={(event) => setCounterAmounts((current) => ({ ...current, [review.auditId]: event.target.value }))} {...stylex.props(styles.input)} /><button type="button" disabled={busy === review.auditId} onClick={() => act(review, "counter")} {...stylex.props(styles.button)}>Counter</button></div></div></article>)}</div></div></>;
}
