...
const styles = stylex.create({
  ...
  closeIcon: { height: 20, position: "relative", width: 20 },
  closeBar: { backgroundColor: "var(--foreground)", borderRadius: 99, display: "block", height: 2, left: 0, position: "absolute", top: 9, width: 20 },
  closeBarFirst: { transform: "rotate(45deg)" },
  closeBarSecond: { transform: "rotate(-45deg)" },
...
export function DashboardNav(): React.JSX.Element {
  ...
  return <header {...stylex.props(styles.header)}><nav aria-label="Primary navigation" {...stylex.props(styles.nav)}><div {...stylex.props(styles.topRow)}><Link href="/" onClick={close} {...stylex.props(styles.brand)}>Reverb Agent Admin</Link><div {...stylex.props(styles.actions)}><ThemeToggle /><button type="button" aria-label={open ? "Close navigation menu" : "Open navigation menu"} aria-expanded={open} onClick={() => setOpen((current) => !current)} {...stylex.props(styles.menuButton)}>{open ? <span aria-hidden="true" {...stylex.props(styles.closeIcon)}><span {...stylex.props(styles.closeBar, styles.closeBarFirst)} /><span {...stylex.props(styles.closeBar, styles.closeBarSecond)} /></span> : <span aria-hidden="true" {...stylex.props(styles.menuIcon)}><span {...stylex.props(styles.menuBar)} /><span {...stylex.props(styles.menuBar)} /><span {...stylex.props(styles.menuBar)} /></span>}</button></div></div>{open ? <div {...stylex.props(styles.links)}><Link href="/listings" onClick={close} {...stylex.props(styles.link)}>Listings</Link><Link href="/rules" onClick={close} {...stylex.props(styles.link)}>Rules</Link><Link href="/offers" onClick={close} {...stylex.props(styles.link)}>Offers</Link><Link href="/escalations" onClick={close} {...stylex.props(styles.link)}>Escalations</Link><Link href="/settings" onClick={close} {...stylex.props(styles.link)}>Settings</Link></div> : null}</nav></header>;
}
