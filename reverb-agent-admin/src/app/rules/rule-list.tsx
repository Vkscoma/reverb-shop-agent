...
export function RuleList({ rules }: { rules: ListingRule[] }): React.JSX.Element {
  ...
  return <table {...stylex.props(styles.table)}><thead><tr>{["Name", "Listing", "Floor", "Target", "Status", "Actions"].map((heading) => <th key={heading} {...stylex.props(styles.cell)}>{heading}</th>)}</tr></thead><tbody>{rules.map((rule) => <tr key={rule.id}><td {...stylex.props(styles.cell)}>{rule.name}</td><td {...stylex.props(styles.cell)}>{rule.listingId}</td><td {...stylex.props(styles.cell)}>{formatUsd(rule.floorPrice)}</td><td {...stylex.props(styles.cell)}>{formatUsd(rule.targetPrice)}</td><td {...stylex.props(styles.cell, rule.isActive ? styles.active : styles.inactive)}>{rule.isActive ? "Active" : "Paused"}</td><td {...stylex.props(styles.cell)}><button type="button" onClick={() => toggle(rule)} {...stylex.props(styles.action)}>{rule.isActive ? "Pause" : "Activate"}</button><button type="button" onClick={() => remove(rule.id)} {...stylex.props(styles.action, styles.danger)}>Delete</button></td></tr>)}</tbody></table>;
}
