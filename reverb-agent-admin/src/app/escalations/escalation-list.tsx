...
export function EscalationList({ items }: { items: Escalation[] }): React.JSX.Element {
  ...
  return <div {...stylex.props(styles.tableWrap)}><table {...stylex.props(styles.table)}><thead><tr>{["Intent", "Conversation", "Message", "Status"].map((heading) => <th key={heading} {...stylex.props(styles.cell)}>{heading}</th>)}</tr></thead><tbody>{items.map((item) => <tr key={item.id}><td {...stylex.props(styles.cell)}>{item.intent}</td><td {...stylex.props(styles.cell)}>{item.conversationId}</td><td {...stylex.props(styles.cell)}>{item.message}</td><td {...stylex.props(styles.cell, styles.status)}><select aria-label={`Status for ${item.conversationId}`} value={item.status} onChange={(event) => setStatus(item.id, event.target.value)} {...stylex.props(styles.select)}><option value="open">Open</option><option value="acknowledged">Acknowledged</option><option value="resolved">Resolved</option></select></td></tr>)}</tbody></table></div>;
}
